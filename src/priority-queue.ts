...
export default class PriorityQueue {
	...
	#heapifyUp() {
		...
		while (index > 0) {
			...
			if (
				(this.#isAsc && this.#heap[index].priority < this.#heap[rootIndex].priority) ||
				(!this.#isAsc && this.#heap[index].priority > this.#heap[rootIndex].priority)
			) {
	...
	#heapifyDown() {
		...
		while (index * 2 + 1 < this.#heap.length) {
			...
			if (
				...
				((this.#isAsc && this.#heap[rightChildIndex].priority < this.#heap[minChildIndex].priority) ||
					(!this.#isAsc && this.#heap[rightChildIndex].priority > this.#heap[minChildIndex].priority))
			) {
				minChildIndex = rightChildIndex;
			}
			if (
				(this.#isAsc && this.#heap[index].priority > this.#heap[minChildIndex].priority) ||
				(!this.#isAsc && this.#heap[index].priority < this.#heap[minChildIndex].priority)
			) {
